...
export class ProductService {
  ...
  constructor(
    ...
    private readonly productImagesRepository = getRepository(ProductImage),
  ...
  async findOne(term: string) {
    ...
    if (!product)
      throw new NotFoundException(
        `Product with the following term "${term}" not found or has already been deleted`,
      );
  ...
  async update(id: string, updateProductDto: UpdateProductDto, user: User ) {
    ...
    if(!product) throw new NotFoundException(`Product with id *${id}* hasn't been found.`)
    
    // Create query runner --- Documentation: https://orkhan.gitbook.io/typeorm/docs/query-runner
  ...
  private handleDBExceptions(error: any) {
    ...
    // Sino, podemos controlar todos los errores centralizados.
